...
class task extends L_mysql{
	...
	public function chk_insert() {
		$data['title']			= chk_input( 'title' , 'get' );
		$data['distribution']	= chk_input( 'distribution' , 'get' );
		$data['author']			= chk_input( 'author' , 'get' );
		$data['recipient']		= chk_input( 'recipient' , 'get' );
		$data['status']			= chk_input( 'status' , 'get' );
		$data['content']		= chk_input( 'content' , 'get' );
		
		$cnt	= count( $data );
		$data	= array_filter( $data );
		
		if( empty( $data ) || ($cnt != count($data)) ) {
			throw new Exception('请把数据填写完整',181);
		}
	...
	public function chk_update() {
		...
		$data['distribution']	= chk_input( 'distribution' , 'post' );
		$data['author']			= chk_input( 'author' , 'post' );
		$data['recipient']		= chk_input( 'recipient' , 'post' );
